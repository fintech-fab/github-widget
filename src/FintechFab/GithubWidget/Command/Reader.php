...
use FintechFab\GithubWidget\Model\Conditions;
use FintechFab\GithubWidget\Model\Events;
use FintechFab\GithubWidget\Model\Issues;
use FintechFab\GithubWidget\Model\Members;
use FintechFab\GithubWidget\Model\ModelInterface;
...
class Reader extends Command
{
	...
	{
		$this->ownerRepo = $repoPath = $this->argument('repo');
		$repoPath = trim($repoPath);
		$repoPath = trim($repoPath, '/');
		list($owner, $repo) = explode('/', $repoPath);


		$this->gitHubAPI = new Api($owner, $repo);
	}
	...
	{
		return array(
			array('repo', InputArgument::REQUIRED, 'repo link like vendor/package'),
		);
	}
	 ...
	 * @param string $option
	 */
	protected function showHelp($option)
	{
	...
	{
		$maxDate = Comments::whereRepo($this->ownerRepo)->max('updated'); //Максимальная дата, полученная с GitHub'а
		...
		$this->processTheData(Comments::class, empty($maxDate) ? 0 : strtotime(str_replace(" ", "T", $maxDate) . "Z"));
	...
	{
		$strMaxDate = Events::whereRepo($this->ownerRepo)->max('created');
		...
		while ($isContinue && $this->gitHubAPI->doNextRequest()) {
			...
			foreach ($this->gitHubAPI->response as $item) {
				if (Events::isAcceptData($item)) {
					$response[] = $item;
				}
			}

			$this->saveInDB($response, Events::class);
		}
	...
	{
		$maxDate = Issues::whereRepo($this->ownerRepo)->max('updated'); //Максимальная дата, полученная с GitHub'а
		//В парметре — запрос на новые данные
		$param = empty($maxDate) ? "" :
			"state=all&since='" . str_replace(" ", "T", $maxDate) . "Z'";

		$this->gitHubAPI->setNewRepoQuery('issues', $param);
		$this->processTheData(Issues::class);
	}
	...
	{
		$maxDateStr = Refcommits::whereRepo($this->ownerRepo)->max('created'); //Максимальная дата в БД
		$maxDate = empty($maxDateStr) ? 0 : strtotime(str_replace(" ", "T", $maxDateStr) . "Z");

		$this->gitHubAPI->setNewRepoQuery("issues/events");
		$this->processTheData(Refcommits::class, $maxDate, 'created_at');

		//Добавление сообщений коммитов
		$refCommits = Refcommits::where('message', '')->get();
	 ...
	 * @param string $group
	 */
	protected function usersData($group)
	{
		...
		if ($this->gitHubAPI->doNextRequest()) {
			$this->info("\nLimit remaining: " . $this->gitHubAPI->getLimitRemaining());
			$this->info("Результат запроса: " . $this->gitHubAPI->messageOfResponse);
			$this->saveInDB($this->gitHubAPI->response, Members::class);
	 ...
	 */
	protected function processTheData($dataModel, $filterDate = 0, $fieldName = 'updated_at')
	{
		$isContinue = true;
		while ($isContinue && $this->gitHubAPI->doNextRequest()) {
			$this->info("\nLimit remaining: " . $this->gitHubAPI->getLimitRemaining());
	 ...
	 */
	protected function timeFilter($filterDate, $fieldName = 'updated_at')
	{
		if ($filterDate == 0) {
			return true;
		}
	 ...
	 */
	protected function prepareCondition($forRepoItem)
	{
		/** @var Eloquent|Conditions $repoCondition */
		$repoCondition = Conditions::whereRepoItem($forRepoItem)->whereRepo($this->ownerRepo)->first();
	 ...
	 * @param string $forRepoItem
	 */
	protected function updateConditionalRequest($forRepoItem)
	{
		if (!$this->gitHubAPI->isDoneRequest()) {
			return;
		}
		...
		if (empty($repoCondition)) {
			$repoCondition = new Conditions();
			$repoCondition->repo_item = $forRepoItem;
			$repoCondition->repo = $this->ownerRepo;
		}
	 ...
	 */
	protected function saveInDB($inData, $classDB)
	{
		$this->info(sprintf("\nAddition to DataBase: %u records...", count($inData)));

		/** @var Eloquent|ModelInterface $item */
		$item = new $classDB;
		$keyNames = $item->getKeyName();
